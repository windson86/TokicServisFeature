...
export class ServisOptions extends Component {
    ...
    continue = e => {
      ...
      else{ toast.warning('Odaberite bar jednu opciju')}
    ...
    handleChange = e =>{
        ...
        this.setState(prevState => ({
            selectedServices: [...prevState.selectedServices, {name:e.target.name,price:e.target.value}]
          }))}
          ...
          if(!e.target.checked){
              ...
              //filtriramo object van
               ...
               this.setState({
                   selectedServices:  this.state.selectedServices.filter((_, i) => i !==selectedServices.findIndex(v => v.name === e.target.name) )
    ...
    render() {
        ...
        const {selectedServices,haveDiscount,textKupona,validKupon}=this.state
            ...
            <ToastContainer />
                <h2>Korak 2. Odaberite jednu ili više usluga za koje ste</h2>
                    ...
                    {ServicesArray.map((servis,index)=>{
                        ...
                        return(
                            <FormGroup >
                            <FormControlLabel control={<Checkbox checked={selectedServices.some(e => e.name === servis.name)} onChange={this.handleChange} value={servis.price} name={servis.name} />} label={servis.name+'( '+servis.price+' kn)'} />
  ...
  {!validKupon&&
                <div>ukupno:{this.fixed(totalCijena)}</div>} 

  {validKupon&&
                
            <div>
               <p style={{ color: 'green' }}>Zahvaljujući kuponu ostvarili ste popust od 30%</p>
                glavnica:{totalCijena.toFixed(2)}<br/>Popust:-{(totalCijena*0.3).toFixed(2)}<br/>Ukupno:{(totalCijena-totalCijena*0.3).toFixed(2)}</div>} 

                  {!haveDiscount&&
                                   <div> <text onClick={this.handleKupon} style={{textDecorationLine: 'underline'} } >Imam kupon</text> <br/></div>
                                }      
                  {haveDiscount&&!validKupon&& 
                    <div>  
                        <TextField
                            ...
                            <button onClick={this.checkForKupon}>Primjeni</button> </div>}  
