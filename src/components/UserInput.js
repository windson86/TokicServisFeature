...
export class UserInput extends Component {
  ...
  continue = (e) => {
    ...
    if (!values.imePrezime) {
      toast.warning("ime i prezime je obavezno polje");
    }
    if (!values.email) {
      toast.warning("email je obavezno polje");
    }
    if (!values.brojTelefona) {
      toast.warning("kontakt broj je obavezno polje");
    }
    if (!validator.isEmail(values.email)) {
      toast.warning("provjerite da li je email ispravno upisan");
    }
    ...
    return (
      <MuiThemeProvider>
        <div>
          ...
          <RaisedButton label="dalje" primary={true} onClick={this.continue} />
        </div>
        <ToastContainer />
      </MuiThemeProvider>
    );
